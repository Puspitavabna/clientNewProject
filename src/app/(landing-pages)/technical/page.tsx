...
import TechnicalCard from "@/src/app/components/CommonCard";
  ...
  return (
    <>
      <header className="flex flex-col gap-y-4 bg-tertiary">
        ...
        <NewHero image={skyfaced} title="TECHNICAL PROJECTS" desc="We deliver innovative solutions in Information Technology and Civil Engineering, creating sustainable and efficient projects worldwide. Our expertise transforms industries, enhances communities, and sets new standards for quality and " />
      </header>

      <div className="px-3 bg-tertiary py-10 ">
        <SliderIcon
          data={[
            <FaReact
      ...
      <main className="bg-gradient-to-br from-[#E9F0FF] to-[#E9F0FF] p-14">
        <div className="mb-20 grid grid-cols-4 gap-4">
          {loading ? <SkeletonCard /> : projectData?.length > 0 ? projectData?.map((d: any, i: number) => (
            <TechnicalCard key={i} data={d} type="technical" />
          )) : <TechnicalCard key={dummyData._id} data={dummyData} type="technical" />}
        </div>
        <Paginate />
      </main>
    </>
